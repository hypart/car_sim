...
Eigen::Vector3f Environment::update_camera(Eigen::Vector3f car_pos, Eigen::Vector3f car_dir,
...
Color Environment::dist_fade(float x, float y) {
...
void Environment::init_trees() {
    for (int i = 0; i < num_trees; i++) {
        float diam = 0.1 + 0.9 * std::abs(rand_uniform());
        float height = 1.0 + 4.0 * std::abs(rand_uniform());
        float xpos = render_dist * rand_uniform();
        float ypos = render_dist * rand_uniform();
        trees.push_back(tree({xpos, ypos, diam, height}));
    }
}

void Environment::update_trees(Eigen::Vector3f cam_pos) {
    for (tree& tree : trees) {
...
Environment::Environment(float _floor_z, float _floor_density, float _render_dist) {
    camera = {0};
    floor_z = _floor_z;
...
void Environment::draw(Eigen::Vector3f car_pos, Eigen::Vector3f car_dir, float car_dist,
                       float cam_tilt) {
    ...
    for (float x = min_x; x < (max_x - ds); x += ds) {
        for (float y = min_y; y < (max_y - ds); y += ds) {
            ...
            Color col = dist_fade(x + 0.5f * ds - cam_pos.x(), y + 0.5f * ds - cam_pos.y());
    ...
    for (tree tree : trees) {
        ...
        DrawCylinderWiresEx(start, end, tree.diam / 2.0f, tree.diam / 2.0f, 10, col);
    }
}