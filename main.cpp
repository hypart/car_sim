...
int main(int argc, const char *argv[]) {
  ...
  carBase car = carBase(1 / (dispFPS * phys_per_disp_FPS), dispFPS, 4.0, 1.75,
                        1500.0f, 0.0005, Eigen::Vector3f(0.0f, 0.0f, 0.0f));
  Environment scene = Environment(0.0f, 1.0f, 100.0f);
  ...
  while (!WindowShouldClose()) {
    ...
    if (IsKeyDown(KEY_DOWN))
      cam_pitch -= 2.0 * std::abs(cam_pitch_lim + cam_pitch) / dispFPS;
    else if (IsKeyDown(KEY_UP))
      cam_pitch += 2.0 * std::abs(cam_pitch_lim - cam_pitch) / dispFPS;
    else
      cam_pitch += cam_alpha * (cam_pitch_default - cam_pitch);

    if (IsKeyDown(KEY_RIGHT))
      cam_yaw -= 2.0 * std::abs(cam_yaw_lim + cam_yaw) / dispFPS;
    else if (IsKeyDown(KEY_LEFT))
      cam_yaw += 2.0 * std::abs(cam_yaw_lim - cam_yaw) / dispFPS;
    else
      cam_yaw += cam_alpha * (cam_yaw_default - cam_yaw);

    if (IsKeyReleased(KEY_E))
      gear_state = std::min(gear_state + 1, car.drivetrain.max_gear);
    if (IsKeyReleased(KEY_Q))
      gear_state = std::max(gear_state - 1, car.drivetrain.min_gear);
    ...
    for (int i = 0; i < phys_per_disp_FPS; i++) {
      car.input_control(throttle, brake, steer, gear_state, clutch_command,
                        starter_command);
      car.update_state();
    }
    ...
    Eigen::Vector3f cam_dir =
        Eigen::AngleAxisf(cam_yaw, Eigen::Vector3f(0.0f, 0.0f, 1.0f)) *
        Eigen::Vector3f(std::cos(car.tc), std::sin(car.tc), 0.0f);
    ...
    DrawText(TextFormat("rpm: %.0f", car.drivetrain.w * 60 / (2 * pi)), 10, 100,
             20, WHITE);
    DrawText(TextFormat("throttle: %.0f%%", car.drivetrain.throttle * 100.0f),
             220, 10, 20, WHITE);
    DrawText(
        TextFormat("clutch: %.0f%% engaged", car.drivetrain.clutch * 100.0f),
        220, 40, 20, WHITE);
    EndDrawing();
