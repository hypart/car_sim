...
public:
  ...
  float brake_per_wheel() { return brake * 10.0f / 4.0f; }

  float drag_force() { return d * vl * std::abs(vl); }
  ...
  float dvl_dt() { return accel() - drag_force() - brake_force(); }
  ...
  std::tuple<float, float> wheel_rots(float curr_ts) {
    ...
    if (std::abs(curr_ts) < steer_eps) {
      ...
      float ti = pi / 2 - std::atan(1 / std::tan(std::abs(curr_ts)) -
                                    w / (2 * l)); // inside wheel angle
      float to = pi / 2 - std::atan(1 / std::tan(std::abs(curr_ts)) +
                                    w / (2 * l)); // outside wheel angle
      if (curr_ts > 0.0) {
        tr = to;
        tl = ti;
      } else {
  ...
  void update_control(float throttle_command, float brake_command,
                      float steer_command, int gear_command,
  ...
  float accel() { // longitudonal acceleration [m/s^2]
    float N =
        drivetrain.final_drive_ratio * drivetrain.ratios[drivetrain.gear + 1];
...
public:
  carBase(float timestep, float disp_fps, float length, float width, float mass,
          float drag, Eigen::Vector3f init_pos) {
    dt = timestep;
  ...
  void input_control(float throttle_command, float brake_command,
                     float steer_command, int gear_command,
                     float clutch_command, int starter_command) {
    ...
    float thr_smooth =
        drivetrain.throttle + thr_a * (throttle_command - drivetrain.throttle);
    float brk_smooth = brake + brk_a * (brake_command - brake);
    float str_smooth = ts + str_a * (steer_command - ts);
    float clt_smooth =
        drivetrain.clutch + clt_a * (clutch_command - drivetrain.clutch);

    update_control(thr_smooth, brk_smooth, str_smooth, gear_command,
                   clt_smooth);
  ...
  void draw() {
    ...
    for (Eigen::Vector3f p : wheel_positions) {
      ...
      DrawCylinderWiresEx(start_v, end_v, wheel_radius, wheel_radius, 100,
                          WHITE);
      wheel_idx++;
    }
  }

  short play_sound() { return sound.emit(drivetrain.throttle, drivetrain.w); }
};
